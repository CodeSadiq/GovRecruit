...
function BulletinEditorContent() {
   ...
   const handlePublish = async () => {
      ...
      try {
         await saveBulletinToRegistry(previewData, bulletinCategory);
         setIsPublishing(false);
         alert('Institutional Manifest Synchronized Successfully.');
         router.push('/admin?tab=bulletin');
      } catch (e) {
         setIsPublishing(false);
         alert('Registry synchronization failed. Check console for details.');
         console.error(e);
      }
   };
   ...
   const suggestedJob = useMemo<any>(() => {
   ...
   const jobSearchResults = useMemo(() => {
      ...
      return publishedJobs.filter(job =>
         ...
         (job.organization && job.organization.toLowerCase().includes(jobSearchQuery.toLowerCase())) ||
         job.id.toLowerCase().includes(jobSearchQuery.toLowerCase())
      ).slice(0, 5);
   }, [jobSearchQuery, publishedJobs]);

   const applyManualLink = (jobId: string) => {
      if (!previewData) return;
      const newData = { ...previewData, routedTo: `/all-jobs/${jobId}` };
   ...
   return (
      <div className="min-h-screen bg-[#FDFDFD] pt-1 pb-10 px-6 lg:pt-2 lg:px-12 font-sans selection:bg-navy selection:text-white">
         <div className="max-w-[1240px] mx-auto space-y-6 animate-in fade-in duration-700">

            {/* HEADER */}
            <div className="flex flex-col md:flex-row items-end justify-between gap-4 pb-4 border-b-2 border-gray-100">
               <div className="space-y-2 text-left">
                  <button
                     onClick={() => router.push('/admin?tab=bulletin')}
                     className="inline-flex items-center gap-2 text-[11px] font-black uppercase tracking-widest text-navy/30 hover:text-navy no-underline bg-transparent border-none cursor-pointer p-0 transition-all font-mono"
                  >
                     ← back
                  </button>
                  <h1 className="text-2xl lg:text-3xl font-black text-navy uppercase leading-none tracking-tighter">
                     Bulletin Publisher
                  </h1>
               </div>
            </div>
                  ...
                  <div className="space-y-6">
                     ...
                     <div className="space-y-3">
                        ...
                        <textarea
                           ...
                           className="w-full h-[200px] bg-gray-50 border border-gray-100 p-4 rounded-xl text-[10px] font-mono font-medium text-navy/70 placeholder:text-navy/10 outline-none focus:border-navy transition-all resize-none shadow-inner"
                        ></textarea>
                     </div>
               ...
               <div className="space-y-6">
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                     ...
                     <button
                        ...
                        className={`px-3 py-1.5 rounded-lg text-[8px] font-black uppercase tracking-widest transition-all border shrink-0 ${isEditMode ? 'bg-navy text-white border-navy' : 'bg-white text-navy/40 border-gray-100 hover:border-navy hover:text-navy'}`}
                     >
                        {isEditMode ? 'Editing Active ●' : 'Editing Locked ○'}
                     </button>
                  </div>

                  <div className="bg-[#FDFDFD] border border-gray-100 rounded-2xl overflow-hidden min-h-[650px] p-6 md:p-10 flex flex-col items-center">
                     {previewData ? (
                        <article className="animate-in fade-in duration-300 w-full max-w-[640px] space-y-5">
                           ...
                           <footer className="pt-4 space-y-4">
                              <div className="flex flex-col md:flex-row items-center gap-4">
                                 ...
                                 {isEditMode && suggestedJob && previewData && previewData.routedTo !== `/all-jobs/${suggestedJob.id}` && !jobSearchQuery && (
                                    ...
                                    <div className="flex-1 space-y-2">
                                       <div className="relative">
                                          <input
                                             ...
                                             placeholder="SEARCH JOB POST TO ADD IN VIEW DETAILS..."
                                             className="w-full bg-white border border-gray-100 p-3 rounded-2xl text-[10px] font-bold text-navy placeholder:text-navy/20 uppercase tracking-widest focus:border-navy transition-all pr-12 shadow-sm"
                                          />
                                          <div className="absolute right-4 top-1/2 -translate-y-1/2 text-navy/20">
                                          ...
                                          {jobSearchResults.length > 0 && (
                                             <div className="absolute bottom-full left-0 right-0 mb-2 bg-white border border-blue-100 rounded-2xl overflow-hidden shadow-2xl z-50 animate-in fade-in zoom-in-95 duration-200">
                                                {jobSearchResults.map((job) => (
                                                   <div key={job.id} className="w-full flex items-center gap-2 border-b border-blue-50/50 last:border-0 hover:bg-blue-50 transition-colors">
                                                      <button
                                                         type="button"
                                                         onClick={() => applyManualLink(job.id)}
                                                         className="flex-1 text-left p-3 bg-transparent border-none"
                                                      >
                                                         <div className="text-[10px] font-black text-navy leading-tight">{job.title}</div>
                                                         <div className="text-[8px] font-bold text-navy/30 mt-1 uppercase tracking-widest">{job.id}</div>
                                                      </button>
                                                      <Link
                                                         href={`/all-jobs/${job.id}`}
                                                         target="_blank"
                                                         className="p-3 text-navy/20 hover:text-navy transition-all"
                                                         title="Preview Job Manifest"
                                                      >
                                                         <IconEye />
                                                      </Link>
                                                   </div>
   ...
   return (
      <Suspense fallback={
         <div className="min-h-screen flex items-center justify-center bg-[#FDFDFD]">
            <div className="text-[10px] font-black uppercase tracking-widest text-navy/20 animate-pulse">Initializing Security Port...</div>
         </div>
      }>
