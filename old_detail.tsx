...
// ΓöÇΓöÇ DATA FETCHING ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
              ...
              {q.extraConditions && q.extraConditions.length > 0 && (
                <div style={{ marginTop: 4 }}>
                  {q.extraConditions.map((c, i) => (
                    <span key={i} className="qual-condition">ΓÜá {c}</span>
                  ))}
                </div>
              )}
...
// ΓöÇΓöÇ PAGE COMPONENT ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
export default async function JobDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // Fee grouping ΓÇö merge categories with identical fee
  ...
  const salaryStr = job.salary?.payLevel
    ? `Pay Level ${job.salary.payLevel} (${fmtMoney(job.salary.min)} ΓÇô ${fmtMoney(job.salary.max)})`
    : (job.salary?.min || job.salary?.max)
      ? `${fmtMoney(job.salary.min)} ΓÇô ${fmtMoney(job.salary.max)}`
      : null;

  return (
    <main className="min-h-screen bg-[#faf8f5]">
      {/* ΓöÇΓöÇ MASTHEAD ΓöÇΓöÇ */}
      {/* PROFESSIONAL NAV */}
      ...
      <div className="article-wrap">
        ...
        {/* ΓöÇΓöÇ VACANCY HERO STRIP ΓöÇΓöÇ */}
        <div className="vacancy-strip">
          <div className="vac-cell highlight">
            <div className="vac-label">Capacity</div>
            <div className="vac-value">{job.totalVacancy?.toLocaleString("en-IN") ?? "ΓÇö"}</div>
          ...
          <div className="vac-cell">
            ...
            <div className="vac-value" style={{ fontSize: 16 }}>
              {job.salary?.min ? fmtMoney(job.salary.min) : "ΓÇö"}
            </div>
          ...
          <div className="vac-cell">
            ...
            <div className="vac-value" style={{ fontSize: 15 }}>
              {(dates as any).lastDate ? fmtDate((dates as any).lastDate) : "ΓÇö"}
            </div>
          </div>
        </div>
        ...
        <table className="info-pair-table">
          <tbody>
            ...
            <tr>
              <td>Total Vacancy</td>
              <td><strong>{job.totalVacancy?.toLocaleString("en-IN") ?? "ΓÇö"}</strong></td>
            </tr>
            <tr>
              <td>Location</td>
            ...
            <tr>
              ...
              <td>{job.exServicemanQuota ? "Γ£à Applicable" : "No"}</td>
            </tr>
          </tbody>
        </table>

        {/* SECTION 05 ΓÇö FEE SCHEDULE */}
        <div className="section-rule">
        ...
        {/* SECTION 02 ΓÇö VACANCY DETAILS */}
        <div className="section-rule">
              ...
              {(job.posts && job.posts.length > 0) ? (
                job.posts.map((p, idx) => (
                  <tr key={idx}>
                    ...
                    <td style={{ fontWeight: 400 }}>{p.totalVacancy?.toLocaleString("en-IN") || "ΓÇö"}</td>
                    <td className="td-left" style={{ fontWeight: 400 }}>
                            ...
                            <li key={qidx}>
                              ...
                              {q.extraConditions && q.extraConditions.length > 0 && (
                                <div style={{ marginTop: 4, color: 'var(--jp-ink-muted)' }}>
                                  {q.extraConditions.map(c => <span key={c}>ΓÇó {c} </span>)}
                                </div>
                              )}
                            </li>
                          ))}
        ...
        {/* SECTION 01 ΓÇö IMPORTANT DATES */}
        <div className="section-rule">
            ...
            <tbody>
              {dateRows.map(row => {
                ...
                if (row.key === "lastDate" && val) {
                  ...
                  else if (days !== null && days <= 7) { cls = "date-soon"; badge = <span className="badge badge-amber">ΓÜá CLOSING</span>; }
                  else { cls = "date-ok"; }
                }
                return (
                  <tr key={row.key}>
        ...
        {/* SECTION 06 ΓÇö SELECTION STAGES */}
              ...
              {job.selectionProcess.map((step, i) => (
                <div key={i} className="sel-item">
                  ...
                  {i < job.selectionProcess.length - 1 && (
                    <span className="sel-arrow">ΓÇ║</span>
                  )}
                </div>
              ))}
            </div>
